...
(defun treesit-auto--sync-langs ()
  "Rebuild `treesit-auto-langs' from `treesit-auto-recipe-list'."
  (setq treesit-auto-langs
        (delete-dups
         (mapcar #'treesit-auto-recipe-lang treesit-auto-recipe-list)))
  treesit-auto-langs)

(defun treesit-auto--remove-recipes (&rest langs)
  "Remove recipes from `treesit-auto-recipe-list' whose :lang is in LANGS.

LANGS should be symbols, e.g. 'janet, 'python."
  (setq treesit-auto-recipe-list
        (cl-remove-if (lambda (recipe)
                        (memq (treesit-auto-recipe-lang recipe) langs))
                      treesit-auto-recipe-list))
  (treesit-auto--sync-langs))

(defun treesit-auto--insert-recipe (&rest args)
  "Create a `treesit-auto' recipe from ARGS and insert it into
`treesit-auto-recipe-list'.

ARGS are the keyword args you would pass to `make-treesit-auto-recipe'.

Example:
  (treesit-auto--insert-recipe
    :lang 'javascript
    :ts-mode 'js-ts-mode
    ...
    :source-dir \"src\"
    :ext \"\\\\.js\\\\'\")"
  (let ((recipe (apply #'make-treesit-auto-recipe args)))
    (add-to-list 'treesit-auto-recipe-list recipe)
    (treesit-auto--sync-langs)
    recipe))

(defun treesit-auto--replace-recipe (&rest args)
  "Replace an existing `treesit-auto' recipe (same :lang) with a new one built from ARGS.

ARGS are passed to `make-treesit-auto-recipe'.

This will:
- Find an existing recipe in `treesit-auto-recipe-list' whose :lang
  matches the :lang in ARGS.
- If found: remove that recipe (via `treesit-auto--remove-recipes')
  and insert the new one (via `treesit-auto--insert-recipe').
- If *no* recipe with that :lang exists: do nothing (noop).

Return the new recipe if replacement happened, or nil otherwise."
  (let* ((lang (plist-get args :lang))
         (existing (and lang
                        (cl-find lang treesit-auto-recipe-list
                                 :key #'treesit-auto-recipe-lang
                                 :test #'eq))))
    (when existing
      (treesit-auto--remove-recipes lang)
      (apply #'treesit-auto--insert-recipe args))))

(defun treesit-auto--kill-candidates-buffer ()
  "If the `*Treesit-auto install candidates*' buffer is open, kill it,
quitting it's window, else noop."
  (when-let ((buf (get-buffer "*Treesit-auto install candidates*"))
